...
# $Id: cvs_merge.sh,v 1.1 2004/08/19 18:49:21 angiuoli Exp $
...
do case "$opt" in
    ...
    \?)   echo "Usage: `basename $0` -r(oot tag) -b(branch tag) [-d(irectory for output)] [-t(est)]";
          echo;
          echo "eg. `basename $0` -r bsml-v1r8 -b bsml-v1r8-branch";
          exit 1;;
  esac
done
...
cd ../branch
cvs -Q co -r $BRANCH_TAG bsml_all
if [ "$?" -ne 0 ]; then
    echo "Bad release tag $BRANCH_TAG for bsml_all"
    exit 1
fi
